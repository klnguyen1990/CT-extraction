...
def generate_merged_file(json_path:str) -> dict:
    ...
    for filename in filenames:
        with open(os.path.join(json_path, filename)) as json_file:
            ...
            studyUID = data['study']['StudyInstanceUID']['py/newargs']['py/tuple'][0]
            studyDetails = data['study']
            studyDetails['StudyInstanceUID'] = studyDetails['StudyInstanceUID']['py/newargs']['py/tuple'][0]
            if isinstance(studyDetails['PatientWeight'],dict): 
                studyDetails['PatientWeight'] = studyDetails['PatientWeight']['py/newargs']['py/tuple'][0]
            if isinstance(studyDetails['PatientHeight'],dict):
                studyDetails['PatientHeight'] = studyDetails['PatientHeight']['py/newargs']['py/tuple'][0]
                
            seriesDetails = data['series']
            
            seriesDetails['SeriesInstanceUID'] = seriesDetails['SeriesInstanceUID']['py/newargs']['py/tuple'][0]
            
            if seriesDetails['SeriesNumber'] is not None: 
                seriesDetails['SeriesNumber'] = seriesDetails['SeriesNumber']['py/newargs']['py/tuple'][0]
            if isinstance(seriesDetails['ConvolutionKernel'],dict):
                seriesDetails['ConvolutionKernel'] = seriesDetails['ConvolutionKernel']['_list']
